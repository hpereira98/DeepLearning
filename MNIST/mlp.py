...
from tensorflow.keras.callbacks import TensorBoard   ####alteração para gerar dados para o tensorboard
...
def mnist_utilizando_mlp():
    (X_train, y_train), (X_test, y_test) = load_mnist_dataset('mnist.npz')
    # transformar a matriz 28*28 das imagens num vector com 784 atributos para cada imagem (porque émultilayer-perceptron)
    ...
    tensorboard = TensorBoard(log_dir='MNIST_MLP', histogram_freq=0, write_graph=True, write_images=True)
    # ann_viz(model, view=True, filename="mnist_mlp.gv", title="MNIST MLP")
    # treinar a rede
...
def mnist_utilizando_mlp_live_plot():
    (X_train, y_train), (X_test, y_test) = load_mnist_dataset('mnist.npz')
...
if __name__ == '__main__':
    ...
    # mnist_utilizando_mlp_live_plot()